 ...
 * 구독 Repository
 */
public interface SubscriptionRepository extends JpaRepository<LearningSubscription, Integer> {

    /**
     * 회원의 구독 목록 조회
     */
    List<LearningSubscription> findByMemberMemberIdOrderByCreatedAtDesc(Integer memberId);

    /**
     * 회원 + 구독 ID로 구독 조회
     */
    Optional<LearningSubscription> findByMemberMemberIdAndSubscriptionId(Integer memberId, Integer subscriptionId);
